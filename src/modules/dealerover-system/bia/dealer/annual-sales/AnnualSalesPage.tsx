...
import { ErrorPage } from "@/app/(dealerover-system)/dealerover-system/_components/ErrorPage";
import { useAnnualSalesFilters } from "./providers/AnnualSalesFilterProvider";
import { useAnnualSales } from "./hooks/useAnnualSales";
import { AnnualSalesFilters } from "./components/AnnualSalesFilters";
import { AnnualSalesKPICards } from "./components/AnnualSalesKPICards";
import { AnnualSalesChart } from "./components/AnnualSalesChart";
import { AnnualSalesTable } from "./components/AnnualSalesTable";
import { AnnualSalesSkeleton } from "./components/AnnualSalesSkeleton";
...
export default function AnnualSalesPage() {
  const { filters } = useAnnualSalesFilters();
  ...
  const { data, isLoading, error, refresh } = useAnnualSales(queryParams);
  ...
  if (isLoading) return <AnnualSalesSkeleton />;
    ...
    <div className="space-y-6 p-4 md:p-8 pt-6 h-full">
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            <h2 className="text-3xl font-bold tracking-tight">
              {monthLabel ? `${monthLabel} ${filters.year}` : `Annual Sales ${filters.year}`}
            </h2>
          ...
          <p className="text-sm text-muted-foreground">
            {!filters.dealerId
              ? "Select a dealer to view its sell-in vs sell-out performance"
              : monthLabel
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full sm:w-auto">
          <AnnualSalesFilters dealers={dealers} customers={customers} suppliers={suppliers} />
          <Button
            variant="ghost"
            size="icon"
            onClick={refresh}
      ...
      <AnnualSalesKPICards summary={summary} isLoading={isLoading} />

      <AnnualSalesChart data={monthlyData} isLoading={isLoading} />
