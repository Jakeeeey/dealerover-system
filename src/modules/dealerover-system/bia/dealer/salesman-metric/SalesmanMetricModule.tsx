...
import { MetricRow, SalesmanMetricFilters, SalesmanOption, DealerOption } from "./types";
...
export default function SalesmanMetricModule() {
  ...
  // Fetch dealers list on mount
  React.useEffect(() => {
    async function loadDealers() {
      try {
        const res = await fetch("/api/dealerover-system/bia/dealer/salesman-metric?mode=dealers");
  ...
  // Fetch salesmen options when selected dates or selected dealer change
  React.useEffect(() => {
    if (filters.dealerId === "") {
      setSalesmen([]);
      return;
    ...
    async function loadLookups() {
      try {
        const queryParams = new URLSearchParams({
          ...
          dealerId: String(filters.dealerId),
        });
        const response = await fetch(`/api/dealerover-system/bia/dealer/salesman-metric?${queryParams.toString()}`);
    ...
    loadLookups();
  }, [filters.startDate, filters.endDate, filters.dealerId]);

  // Fetch metrics data when filters change
  React.useEffect(() => {
    if (debouncedFilters.dealerId === "") {
      setRows([]);
      return;
    }
    let active = true;
    ...
    async function fetchMetrics() {
      ...
      try {
        ...
        const queryParams = new URLSearchParams({
          ...
          dealerId: String(debouncedFilters.dealerId),
        });

        const res = await fetch(`/api/dealerover-system/bia/dealer/salesman-metric?${queryParams.toString()}`, {
  ...
  }, [debouncedFilters.dealerId, debouncedFilters.salesmanId, debouncedFilters.startDate, debouncedFilters.endDate, salesmen]);

  const handleDealerChange = (id: number | "") => {
    setFilters((prev) => ({
      ...prev,
      dealerId: id,
      salesmanId: "",
  ...
  return (
    <div className="space-y-6">
      ...
      <FiltersBar
        dealers={dealers}
        selectedDealerId={filters.dealerId}
        onChangeDealer={handleDealerChange}
        salesmen={salesmen}
