...
export const fetchSupervisorMappings = async (): Promise<SupervisorKPIResponse> => {
    const url = `/api/dealerover-system/bia/dealer/dealerover-kpi/mapping`;
...
export const fetchSalesmanData = async (startDate: string, endDate: string): Promise<VSalesPerformanceDataDto[]> => {
    ...
    const url = `/api/dealerover-system/bia/dealer/dealerover-kpi/performance?${params.toString()}`;
    const res = await fetch(url, { cache: "no-store" });
    if (!res.ok) return [];
    return await res.json();
};
...
export const fetchDynamicTargets = async (startDate: string, endDate: string): Promise<TargetSettingResponse> => {
    const params = new URLSearchParams({ startDate, endDate });
    const url = `/api/dealerover-system/bia/dealer/dealerover-kpi/targets?${params.toString()}`;
    const res = await fetch(url, { cache: "no-store" });
...
export const fetchCustomerPeaks = async (names: string[] = [], salesmanIds: number[], viewType: 'customer' | 'area' | 'storeType' = 'customer', storeType?: string): Promise<Record<string, { total: number; peak: number; metadata?: Record<string, unknown> }>> => {
    let url = `/api/dealerover-system/bia/dealer/dealerover-kpi/customer-peak?ids=${salesmanIds.join(",")}${names.length > 0 ? `&names=${encodeURIComponent(names.join("|"))}` : ""}&viewType=${viewType}`;
...
export const fetchCustomerTargets = async (salesmanIds: number[], startDate: string, endDate: string, viewType: 'customer' | 'area' | 'storeType' = 'customer', names: string[] = []): Promise<Record<string, number>> => {
    try {
        const url = `/api/dealerover-system/bia/dealer/dealerover-kpi/customer-targets?salesmanIds=${salesmanIds.join(",")}&startDate=${startDate}&endDate=${endDate}&viewType=${viewType}${names.length > 0 ? `&names=${encodeURIComponent(names.join("|"))}` : ""}`;
        const res = await fetch(url, { cache: "no-store" });
        if (!res.ok) return {};
...
export const fetchCustomerProducts = async (
    ...
    const url = `/api/dealerover-system/bia/dealer/dealerover-kpi/customer-products?${params.toString()}`;
    const res = await fetch(url, { cache: "no-store" });
    if (!res.ok) return [];
    return await res.json();
};
