...
function SupervisorKPIContent() {
    ...
    const { matrix, personnel, suppliers, maxAmount, totalsMap, supplierTotals } = useMemo(() => {
        ...
        mappings.supervisors.forEach(s => {
            supervisorIdToName.set(s.id, `${s.supervisor_id.first_name} ${s.supervisor_id.last_name}`.trim());
        });
        ...
        rawData.forEach(item => {
            ...
            const supervisorName = supervisorId ? (supervisorIdToName.get(supervisorId) || "No Assigned Dealer") : "No Assigned Dealer";
        ...
        targets.forEach(t => {
            ...
            const salesmanCode = salesmanIdToCode.get(salesmanId) || rawData.find(d => Number(d.salesmanId) === salesmanId)?.salesmanName || `Sub-dealer/Retailer #${salesmanId}`;
            const pName = selectedSupervisor ? salesmanCode : supervisorName;
            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
                <div className="space-y-1">
                    <div className="flex items-center gap-2 text-muted-foreground text-sm uppercase tracking-widest">
                        ...
                        <span className="cursor-pointer hover:text-primary" onClick={() => setSelectedSupervisor(null)}>Dealer Matrix</span>
                        {selectedSupervisor && (
                            <>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <h2 className="text-4xl font-black tracking-tighter uppercase italic">
                            {selectedSupervisor ? "Sub-dealer/Retailer Performance" : "Dealerover KPI Map"}
                        </h2>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    <div className="relative">
                        ...
                        <Input placeholder={selectedSupervisor ? "Filter sub-dealers/retailers..." : "Filter dealers..."} className="pl-10 w-[180px] bg-card h-10 border-muted-foreground/20" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                    </div>
                                ...
                                <tr className="bg-muted/20">
                                    <th className="sticky left-0 top-0 z-50 bg-background/95 backdrop-blur-sm p-6 text-left border-b border-r min-w-[240px]">
                                        <p className="text-[10px] uppercase tracking-[0.3em] font-black text-primary">
                                            {selectedSupervisor ? "Personnel" : "Dealer"}
                                        </p>
                                    </th>
